...
def analyze_content_with_gemini(pdf_path, page_number, use_image=True, api_key=None):
...
def post_process_paragraphs_from_list(paragraphs_list, api_key):
    ...
    processed_results = [page_paragraphs.copy() if page_paragraphs else [] for page_paragraphs in paragraphs_list]
    ...
    for page in range(len(processed_results)):
        ...
        while i < len(processed_results[page]):
            ...
            if current_text.rstrip().endswith(':') or current_text.rstrip().endswith('：'):
                ...
                elif page + 1 < len(processed_results) and processed_results[page + 1]:
                ...
                if next_text is not None:
                    ...
                    if is_bullet_or_numbered_list(next_text):
                        print(f"發現冒號結尾段落後跟隨列表項: 頁 {page} 段落 {i}" + (" (下一頁)" if is_next_page else ""))
                        ...
                        if not is_next_page:
                            ...
                            while continue_merging and i + 1 < len(processed_results[page]):
                                next_text = processed_results[page][i + 1].strip()
                                ...
                                if (is_bullet and '•' in merged_content) or \
                                   (current_number is not None and next_number is not None and next_number == current_number + 1):
                                    # 繼續合併
                                    merged_content = processed_results[page][i] + "\n" + next_text
                                    processed_results[page][i] = merged_content
...
def process_pdf_page(pdf_path, page_number, use_image=True, min_length=30, api_key=None):
    """處理 PDF 頁面並使用 Gemini 進行分析"""
    print(f"正在處理 PDF: {pdf_path}, 頁面: {page_number}")
    
    # 分析內容
    analysis = analyze_content_with_gemini(pdf_path, page_number, use_image, api_key)
...
if __name__ == "__main__":
    # 參數設定
    pdf_path = "./PDF_reader/pdf_file/中央論文1.pdf"  # 替換為你的PDF路徑
    api_key = "api_key"  # 替換為你的API金鑰
